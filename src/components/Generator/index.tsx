...
const parseTargets = (input: string): Target[] => {
...
const generateSurveyRoute = (targets: Target[], speed: number): string[] => {
...
const SurveyRouteGenerator: FC = () => {
    const { register, control, watch, setValue, handleSubmit } = useForm<Values>({
        defaultValues: {
            speed: 2
        }
    });
    const [input, route] = watch(['input', 'route']);
        ...
        <Container maxW="full">
            <VStack align="stretch">
                ...
                <VStack gap={4} as="form" align="stretch" onSubmit={handleSubmit((data) => {
                    const routeLines = generateSurveyRoute(data.contacts, data.speed);
                    setValue('route', routeLines.join('\n'));
                })}>
                    <Field.Root>
                        <Field.Label>Target Input (Lat, Lon, Type, ID)</Field.Label>
                        <Textarea
                            rows={5}
                            {...register('input')}
                    ...
                    <Grid gridTemplateColumns="repeat(7, 1fr)" gap={2}>
                        <Grid templateColumns="subgrid" gridColumn="1 / -1">
                            {['Lat', 'Lon', 'Type', 'ID', 'Tracks', 'Spacing', 'Bearing'].map(h => (
                                <GridItem key={h}><Heading size="sm">{h}</Heading></GridItem>
                            ))}
                        </Grid>
                        {fields.map((field, i) => (
                            <Grid key={field.id} templateColumns="subgrid" gridColumn="1 / -1">
                                <Field.Root><Input size="sm" type="number" step="any" {...register(`contacts.${i}.lat`, {valueAsNumber: true})} /></Field.Root>
                                <Field.Root><Input size="sm" type="number" step="any" {...register(`contacts.${i}.lon`, {valueAsNumber: true})} /></Field.Root>
                    ...
                    <Button type="submit" colorPalette="pink">Generate Route</Button>

                    <Clipboard.Root value={route}>
                        <Clipboard.Label textStyle="label">Mission plan</Clipboard.Label>
                        <InputGroup endElement={<ClipboardIconButton />}>
                            <Clipboard.Input asChild>
                                <Textarea variant="subtle" rows={20} fontFamily="mono" value={route} readOnly />
                            </Clipboard.Input>
                        </InputGroup>
                    </Clipboard.Root>
                </VStack>
