...
import {F250RoadListAppModel, F250RoadListUIModel} from "@/models/mamba";
...
import Summary from "@/components/RoadLists/F250Next/Summary";
import {BiPlus} from "react-icons/bi";
import Record from "@/components/RoadLists/F250Next/Record";
import {upsertDoc} from "@/db";
import DatePicker from "react-datepicker";

type F250NextProps = {
    model: F250RoadListAppModel
    onBeforeSubmit: () => void;
    onAfterSubmit: () => void;
}

const F250Next: FC<F250NextProps> = ({ model, onBeforeSubmit, onAfterSubmit }) => {
    const methods = useForm<F250RoadListAppModel>({
        defaultValues: model,
    });
    ...
    const [cumulative, setCumulative] = useState<F250RoadListUIModel>(() => {
        return calculateCumulative({
            ...model,
            itineraries, startHours, startFuel
        }) as F250RoadListUIModel;
    })
    ...
    const handleFormSubmit = useCallback(async (data: F250RoadListAppModel) => {
        onBeforeSubmit();
